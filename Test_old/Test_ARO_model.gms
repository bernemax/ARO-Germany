...
inv_new_M(l)                    decision variable regarding investment in a prospective line
...
MP_Objective(vv)..                                                                              O_M  =e= sum(l, inv_new_M(l) * I_costs_380(l)) + ETA
;

MP_IB(vv)..                                                                                     IB   =g= sum(l, inv_new_M(l) * I_costs_380(l))
;
...
;
MP_PG_PV(sun,sr,n,t,vv)$(MapSR(n,sr) and MapRes(sun,n) and (ord(vv) lt (itaux+1)))..            PG_M_PV(sun,t,vv)      =l= Cap_res(sun) * AF_M_PV_fixed(t,sr,n,vv) 
;
MP_PG_Wind(wind,wr,n,t,vv)$(MapWR(n,wr) and MapRes(wind,n) and (ord(vv) lt (itaux+1)))..        PG_M_Wind(wind,t,vv)    =l= Cap_res(wind) * AF_M_Wind_fixed(t,wr,n,vv)
;
...
MP_PF_PROS_Cap_UB(l,t,vv)$(pros_l(l) and (ord(vv) lt (itaux+1)))..                              PF_M(l,t,vv) =l= L_cap_inv_380(l) * inv_new_M(l)
;
MP_PF_PROS_Cap_LB(l,t,vv)$(pros_l(l) and (ord(vv) lt (itaux+1)))..                              PF_M(l,t,vv) =g= - L_cap_inv_380(l) * inv_new_M(l)
;
MP_PF_PROS_LIN_UB(l,t,vv)$(pros_l(l) and (ord(vv) lt (itaux+1)))..                              (1-inv_new_M(l)) *M   =g= PF_M(l,t,vv) - B_prosp_380(l) * (sum(n$Map_Send_l(l,n),  Theta(n,t,vv)) - sum(n$Map_Res_l(l,n),  Theta(n,t,vv)))
;
MP_PF_PROS_LIN_LB(l,t,vv)$(pros_l(l) and (ord(vv) lt (itaux+1)))..                              -(1-inv_new_M(l)) *M  =l= PF_M(l,t,vv) - B_prosp_380(l) * (sum(n$Map_Send_l(l,n),  Theta(n,t,vv)) - sum(n$Map_Res_l(l,n),  Theta(n,t,vv)))
;
...
;
SUB_UB_PG_wind(wind)..                                                 sum(t, z_PG_Wind(wind,t))   =l= Gamma_PG_Wind 
;
...
*O_M.l
;
inv_cost_master = sum(l,  inv_new_M.l(l)* I_costs_380(l));
...
UB = min(UB, (sum(l, inv_new_M.l(l)* I_costs_380(l)) + O_Sub.l))
;
...
execute_unload "check_ARO_Test_old.gdx"
;
